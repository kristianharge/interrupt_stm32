...
1. Initialise the TIM low level resources by implementing the following functions depending from feature used :
...
1. Initialise the GPIO interrupts:
In order to initialise the GPIO interrupts, I created the following function:
```c
...
2. Initialise the debounce filter:
In order to initialise the debounce filter, I created the following function:
```c
...
```
This function allows adding a previously defined callback function to the specified GPIO pin.

### How is built the library

Basically what our code does is that it creates an interrupt thanks to the HAL library, and when the interrupt occurs, it launches a timer that will elapse after a previously set period and then execute the callback function.
...
I also added a bool array (`static bool enabled_callbacks[NUM_GPIOS];`) that contains all the GPIO pins that have an enabled callback function; this avoids calling a callback function if the interrupt was enabled but not the callback function.
...
Once the timer elapses, we execute the callback function and re-enable the interrupt.
